...
def get_prime_system_prompt(agent_key: str, default_telos: str = "") -> str:
    """Returns the comprehensive, production-grade system prompt for any 28 Prime Agent key."""
    clean_key = agent_key.lower().replace("_", "-").split("-")[0]
    # Check exact key or prefix match
    for k in PRIME_AGENT_SYSTEM_PROMPTS:
        if k in agent_key.lower():
            return PRIME_AGENT_SYSTEM_PROMPTS[k]

    return f"You are a specialized agent operating in the 1B Scale Agent Civilization. Core Telos: {default_telos}"
